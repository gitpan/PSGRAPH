...
sub setPS {
	...
	if($graphic eq '2Dpie'){
		if((defined($subtype) && $subtype==1) || !defined($subtype)){
			$self->{PS} = 'gsave /radius 125 def /slicecount 1 def /pieslice {/label exch def /endangle exch def /startangle exch def /k exch def /y exch def /m exch def /c exch def /calloutline 29 def /labelpos 33 def 0 0 0 1	setcmykcolor .5 setlinewidth gsave 0 0 moveto /halfangle startangle endangle add 2 div def slicecount 3 mod 0 eq{/calloutline 15 def /labelpos 19 def}if slicecount 3 mod 1 eq{/calloutline 22 def /labelpos 26 def}if halfangle startangle eq {halfangle rotate} {halfangle startangle gt {halfangle rotate}{/halfangle halfangle 180 add def halfangle rotate}ifelse}ifelse /checkangle halfangle def checkangle 360 gt {/checkangle checkangle 360 sub def}if checkangle 90 gt { checkangle 110 lt{ /labelpos labelpos 4 add def }if checkangle 250 lt {checkangle 110 gt {/labelpos labelpos 8 add def}if}if}if checkangle 250 gt {checkangle 290 lt {/labelpos labelpos 14 add def}if}if radius calloutline add 0 lineto stroke grestore halfangle cos radius labelpos add mul typesize 3 div sub halfangle sin radius labelpos add mul typesize 3 div sub moveto /Helvetica-Bold findfont typesize scalefont setfont '
		}elsif($subtype==2){
...
sub writeGraphic {
	...
	if($self->{Graphic} eq '2Dpie'){
		#translate data into degrees in a cercle for pie
		...
		if(defined($self->{Data}) && defined($self->{LabelandColor})){
			...
			if((defined($self->{Subtype}) && $self->{Subtype}==1) || !defined($self->{Subtype})){
				return &pie1($self->{PS}, $transdata, $self->{LabelandColor}, $self->{Hscale}, $self->{Vscale}, $self->{Gexport}, $self->{Valuetype});
			}elsif((defined($self->{Subtype}) && $self->{Subtype}==2)){
				return &pie2($self->{PS}, $transdata, $self->{LabelandColor}, $self->{Hscale}, $self->{Vscale}, $self->{Gexport}, $self->{Valuetype});
			}elsif(defined($self->{Subtype}) && ($self->{Subtype}==3 || $self->{Subtype}==4)){
				if(!defined($self->{Legend})){$self->setLegend('right');}
				my $leg=$self->getLegend;
				if($leg eq 'right' || $leg eq 'left' || $leg eq 'bottom'){
					if($self->{Subtype}==3){
                 		return &pie3($self->{PS}, $transdata, $self->{LabelandColor}, $self->{Hscale}, $self->{Vscale}, $self->{Gexport}, $self->getLegend);
					}elsif($self->{Subtype}==4){
						return &pie4($self->{PS}, $transdata, $self->{LabelandColor}, $self->{Hscale}, $self->{Vscale}, $self->{Gexport}, $self->getLegend, $self->{Valuetype});
					}
				}else{
...
sub pie1{
my ($ps, $data, $labelandcolor, $hscale, $vscale, $gexport, $valuetype) = @_; 
my $slicecounter; my $piefile; my $piececount; my $thisdate = scalar localtime; my $piedirectory="pies/"; my @slice; my $slicecnt; my $startingangle=120; my $piefileextension="eps"; my $hsize=$hscale; my $vsize=$vscale;  my $yend; my $labelmax=0; my $labelx; open(LABELANDCOLOR, "<$labelandcolor") or die("Couldn't open $labelandcolor\n"); while(<LABELANDCOLOR>) {$_=~s/\t.*//; $_=~s/ *$//; chomp; $labelx=length($_); if($labelx>$labelmax){$labelmax=$labelx; } $piececount++;} close(LABELANDCOLOR); my $xstart=306-($hsize*350/2)-($labelmax*$hsize); my $xend=306+($hsize*350/2)+($labelmax*$hsize); my $ystart=396-($vsize*321/2)-($labelmax*$vsize); $yend=396+($vsize*321/2)+($labelmax*$vsize); my @lclines; my $lclines; my $label; my $c; my $m; my $y; my $k; my $thisendangle; my $thisslice; my $thisstartangle; my $filewithdir; my $thispiefile; open(PIECHARTDATA, "<$data") || die "Couldn't open $data\n"; while(<PIECHARTDATA>) {chomp; @slice=split/\t/,$_; $slicecnt=@slice; $piefile=$slice[$slicecnt-1]; $thisslice=$slice[0]; $slicecounter=0; $thispiefile=$piefile.".".$piefileextension; $filewithdir=$piedirectory.$thispiefile; open(PIE, ">$filewithdir") or die("Couldn't create output file: ".$thispiefile); print PIE "%!PS-ADOBE 3.0 EPSF-3.0\n"; print PIE "%%Title: ".$thispiefile."\n"; print PIE "%%Creator: createpies.pl (c)Ken Owen 1999-2014\n"; print PIE "%%Creationdate: ".$thisdate."\n"; print PIE "%%BoundingBox: $xstart $ystart $xend $yend\n\n"; print PIE "/typesize " . $valuetype . " def\n"; print PIE $ps; print PIE "gsave 1 $hscale div 1 $vscale div scale label dup stringwidth pop 2 div -1 mul 2 rmoveto show grestore c m y k setcmykcolor 2 setlinejoin 0 0 moveto 0 0 radius startangle endangle arc closepath gsave fill grestore 0 0 0 0 setcmykcolor 1.5 setlinewidth stroke /slicecount slicecount 1 add def} def\n"; print PIE "\n%%set scale and translation\n"; print PIE "306 396 translate $hsize $vsize scale\n"; $thisstartangle=$startingangle; open(LABELANDCOLOR, "<$labelandcolor") or die("Couldn't open $labelandcolor"); my $counter=0; while (<LABELANDCOLOR>) { chomp; $lclines[$counter]=$_; $counter++; } foreach $lclines(@lclines){ ($label, $c, $m, $y, $k)=split/\t/,$lclines; print PIE "%%draw pie chart\n"; $thisendangle=$thisstartangle+eval($thisslice); if(eval($thisslice)>0){ print PIE "$c $m $y $k $thisstartangle $thisendangle ($label) pieslice\n"; } $thisstartangle=$thisendangle; $slicecounter++; $thisslice=$slice[$slicecounter]; } my $exportstr="convert $filewithdir -density 1200x1200 $piedirectory$piefile.$gexport"; print PIE "\n"; print PIE "showpage\n"; print PIE "grestore\n"; print PIE "%%trailer\n"; print PIE "%%EOF"; close LABELANDCOLOR; close PIE; if(length($gexport)>0){`$exportstr`;} }
}
sub pie2{
my ($ps, $data, $labelandcolor, $hsize, $vsize, $gexport, $valuetype) = @_;
my $startingangle=60; my $piefileextension="eps"; my $xstart=306-($hsize*350/2); my $xend=306+($hsize*350/2 + 10); my $ystart=396-($vsize*350/2) - 8; my $yend=396+($vsize*350/2); my @slice; my $slice; my $piefile; my $piedirectory="pies/"; my $thisdate = scalar localtime; my $thisslice; my $slicecounter; my $filewithdir; my $thisstartangle; my $thisendangle; my $percent1; my $no_slices; my $piename; open(PIECHARTDATA, "<$data") or die("Couldn't open $data"); while (<PIECHARTDATA>) {chomp; @slice=split(/\t/,$_); $no_slices=@slice; $piefile=$slice[$no_slices-1]; $piename=$piefile; $piefile.="\.".$piefileextension; $thisslice=$slice[0]; $slicecounter=0; $filewithdir=$piedirectory.$piefile; open(PIE, ">$filewithdir") or die("Couldn't create output file: ".$piefile); print PIE "%!PS-ADOBE 3.0 EPSF-3.0\n"; print PIE "%%Title: ".$piefile."\n"; print PIE "%%Creator: createpies.pl (c)Ken Owen 1999-2014\n"; print PIE "%%Creationdate: ".$thisdate."\n"; print PIE "%%BoundingBox: $xstart $ystart $xend $yend\n\n"; print PIE "/typesize " . $valuetype . " def\n"; print PIE $ps; print PIE "%%set scale and translation\n"; print PIE "306 396 translate $hsize $vsize scale\n"; $thisstartangle=$startingangle; open(LABELANDCOLOR, "<$labelandcolor") or die("Couldn't open $labelandcolor"); my $counter=0; my @lclines; my $lclines; while (<LABELANDCOLOR>) {chomp; $lclines[$counter]=$_; $counter++; } $counter=@lclines; my $labelx=0; my $labely=$counter*$vsize*9+50; $counter=0; my $wholepercent=0; my $percent1num=0; foreach $lclines(@lclines){my ($label, $c, $m, $y, $k)=split(/\t/,$lclines); print PIE "%%draw pie chart\n"; $thisendangle=$thisstartangle+eval($thisslice); if(eval($thisslice)>0){$percent1num=int(100*eval($thisslice)/360); if(((100*eval($thisslice)/36) % 10) >= 5){$percent1num++;} $percent1=substr($percent1num,0)."%"; if($percent1num == 0){$percent1num=1;} $wholepercent+=$percent1num; if($wholepercent == 101){$percent1num--; $percent1=substr($percent1num,0) . "%"; } print PIE "$c $m $y $k $thisstartangle $thisendangle ($label) ($percent1) pieslice\n"; } $thisstartangle=$thisendangle; $slicecounter++; $thisslice=$slice[$slicecounter]; } my $exportstr="convert $filewithdir -density 1200x1200 $piedirectory$piename.$gexport";  print PIE "\n"; print PIE "showpage\n"; print PIE "grestore\n"; print PIE "%%trailer\n"; print PIE "%%EOF"; close LABELANDCOLOR; close PIE; if(length($gexport)>0){`$exportstr`;} }
}
sub pie3{
my ($ps, $data, $labelandcolor, $hsize, $vsize, $gexport, $legend) = @_; my $piececount=0; my @slice; my $piename; my $slicecnt; my $filecounter; my @pclines;  my $c; my $y; my $m; my $k; my $pclines; my $piefile; my $piedirectory; my $thisslice; my $thispiefile; my $leftright;  my $lclines; my @lclines; my $slicecounter; my $thisdate; my $filewithdir; my $thisstartangle; my $thisendangle;  my $label; my $startingangle=60; my $piefileextension="eps"; my $legend_right;  my $boundingxend; my $boundingxstart;  my $xstart; my $xend; my $ystart; my $yend; my $labelmax=0; my $labelx;  open(LABELANDCOLOR, "<$labelandcolor") or die("Couldn't open $labelandcolor\n");  while(<LABELANDCOLOR>) { $_=~s/\t.*//; $_=~s/ *$//;  chomp;  $labelx=length($_);  if($labelx>$labelmax){$labelmax=$labelx; } $piececount++;} close(LABELANDCOLOR);  if($legend eq "right"){  $boundingxend=306+($hsize*321/2)+($labelmax*9)*$hsize;  $boundingxstart=306-($hsize*321/2);  $xstart=306-($hsize*260/2);  $xend=306+($hsize*260/2);  $ystart=396-($vsize*308/2); $yend=396+($vsize*308/2);  }elsif($legend eq "left"){ $boundingxstart=306-($hsize*321/2)-($labelmax*9)*$hsize;  $boundingxend=306+($hsize*321/2)*$hsize;  $xstart=306-($hsize*260/2);  $xend=306+($hsize*260/2);  $ystart=396-($vsize*308/2);  $yend=396+($vsize*308/2);  }elsif($legend eq "bottom"){ $boundingxstart=306-($hsize*315/2);  $boundingxend=306+($hsize*315/2);  $xstart=306-($hsize*315/2);  $xend=306+($hsize*315/2);  $ystart=396-150-($piececount)*($vsize*9);  $yend=396+($vsize*130);  }else{  $xstart=306-($hsize*315/2);  $xend=306+($hsize*315/2);  $ystart=396-($vsize*315/2)-30;  $yend=396+($vsize*350/2); }  open(PIECHARTDATA, "<$data") or die("Couldn't open $data");  my $counter=0;  while (<PIECHARTDATA>) {  $pclines[$counter]=$_;  $counter++;  }  foreach $pclines(@pclines){  chomp;  @slice=split(/\t/,$pclines);  chomp($slice[2]);  $slicecnt=@slice;  $piefile=$slice[$slicecnt-1];  chomp($piefile);  $piename=$piefile;  $piefile.="\.".$piefileextension;  $piedirectory="pies/";  $thisdate = scalar localtime;  $thisslice=$slice[0];  $slicecounter=0;  $filewithdir=$piedirectory.$piefile;  open(PIE, ">$filewithdir") or die("Couldn't create output file: ".$thispiefile);  print PIE "%!PS-ADOBE 3.0 EPSF-3.0\n";  print PIE "%%Title: ".$piefile."\n";  print PIE "%%Creator: createpies.pl (c)Ken Owen 1999-2014\n";  print PIE "%%Creationdate: ".$thisdate."\n";  if($legend eq "right" || $legend eq "left") {      print PIE "%%BoundingBox: $boundingxstart $ystart $boundingxend $yend\n\n";  }elsif($legend eq "bottom") {  print PIE "%%BoundingBox: $boundingxstart $ystart $boundingxend $yend\n\n";  }else{  print PIE "%%BoundingBox: $xstart $ystart $xend $yend\n\n";  } print PIE "<< /PageSize [1000 1000] >> setpagedevice \n gsave\n";  print PIE $ps;  print PIE "%%set scale and translation\n";  print PIE "306 396 translate $hsize $vsize scale\n";  $thisstartangle=$startingangle;  open(LABELANDCOLOR, "<$labelandcolor") or die("Couldn't open $labelandcolor\n");  $counter=0;  while (<LABELANDCOLOR>) {  chomp;  $lclines[$counter]=$_;  $counter++;  }  $counter=@lclines;  my $labelx=0;  my $labely=$counter*$vsize*9;  $counter=0;  foreach $lclines(@lclines){  ($label, $c, $m, $y, $k)=split(/\t/,$lclines);  print PIE "%%draw pie chart\n";     $thisendangle=$thisstartangle+eval($thisslice);  print "thisslice=$thisslice\n"; if(eval($thisslice)>0){  print PIE "$c $m $y $k $thisstartangle $thisendangle ($label) pieslice\n";   if($legend eq "bottom"){  $leftright=0;  $labelx=-100;  $labely=-150-($slicecounter*$vsize*50);  print PIE "$c $m $y $k ($label) $labelx $labely $leftright  $hsize $vsize legend\n";  $thisstartangle=$thisendangle; $thisslice=$slice[$slicecounter];  }elsif($legend eq "right"){  $labelx=(260/2) + 10;  $labely=$labely-28;  $counter++;  print PIE "$c $m $y $k ($label) $labelx $labely $hsize $vsize legend_right\n";  }elsif($legend eq "left"){ $labelx=-((260/2) + 10);  $labely=$labely-28; $counter++;  print PIE "$c $m $y $k ($label) $labelx $labely $hsize $vsize legend_left\n";  }  } $thisstartangle=$thisendangle; $slicecounter++; $thisslice=$slice[$slicecounter];  }  my $exportstr="convert $filewithdir -density 1200x1200 $piedirectory$piename.$gexport";  print PIE "\n";  print PIE "showpage\n";  print PIE "grestore\n";  print PIE "%%trailer\n";  print PIE "%%EOF";  if(length($gexport)>0){ `$exportstr`;  } }  close LABELANDCOLOR;  close PIE;
}
sub pie4{
my ($ps, $data, $labelandcolor, $hsize, $vsize, $gexport, $legend, $valuetype) = @_; 
my $piececount=0; my @slice; my $piename; my $slicecnt; my $filecounter; my @pclines; my $c; my $y; my $m; my $k; my $pclines; my $piefile; my $piedirectory; my $thisslice; my $thispiefile; my $leftright; my $lclines; my @lclines; my $slicecounter; my $thisdate; my $filewithdir; my $thisstartangle; my $thisendangle; my $label; my $startingangle=60; my $piefileextension="eps"; my $legend_right;  my $boundingxend; my $boundingxstart; my $xstart; my $xend; my $ystart; my $yend; my $labelmax=0; my $labelx; my $percent1num; my $percent1; my $wholepercent; open(LABELANDCOLOR, "<$labelandcolor") or die("Couldn't open $labelandcolor\n"); while(<LABELANDCOLOR>) {$_=~s/\t.*//; $_=~s/ *$//; chomp; $labelx=length($_); if($labelx>$labelmax){$labelmax=$labelx; } $piececount++;} close(LABELANDCOLOR); if($legend eq "right"){ $boundingxend=306+($hsize*321/2)+($labelmax*9+30)*$hsize; $boundingxstart=306-(30+$hsize*321/2); $xstart=306-($hsize*260/2); $xend=306+($hsize*260/2); $ystart=396-($vsize*308/2)-($vsize*30); $yend=396+($vsize*308/2)+($vsize*30); }elsif($legend eq "left"){$boundingxstart=306-($hsize*321/2)-($labelmax*9+30)*$hsize; $boundingxend=306+(30+$hsize*321/2)*$hsize; $xstart=306-($hsize*260/2); $xend=306+($hsize*260/2); $ystart=396-($vsize*308/2)-($vsize*30); $yend=396+($vsize*308/2)+($vsize*30); }elsif($legend eq "bottom"){$boundingxstart=306-($hsize*315/2)-30; $boundingxend=306+($hsize*315/2)+30; $xstart=306-($hsize*315/2); $xend=306+($hsize*315/2); $ystart=396-200-($piececount)*($vsize*9); $yend=396+($vsize*180); }else{ $xstart=306-($hsize*315/2); $xend=306+($hsize*315/2); $ystart=396-($vsize*315/2)-30; $yend=396+($vsize*350/2);} open(PIECHARTDATA, "<$data") or die("Couldn't open $data"); my $counter=0; while (<PIECHARTDATA>) {$pclines[$counter]=$_; 	$counter++; } foreach $pclines(@pclines){chomp;	@slice=split(/\t/,$pclines); chomp($slice[2]); $slicecnt=@slice; $piefile=$slice[$slicecnt-1]; chomp($piefile); $piename=$piefile; $piefile.="\.".$piefileextension; $piedirectory="pies/"; $thisdate = scalar localtime; $thisslice=$slice[0]; $slicecounter=0; $filewithdir=$piedirectory.$piefile; open(PIE, ">$filewithdir") or die("Couldn't create output file: ".$thispiefile); print PIE "%!PS-ADOBE 3.0 EPSF-3.0\n"; print PIE "%%Title: ".$piefile."\n"; print PIE "%%Creator: createpies.pl (c)Ken Owen 1999-2014\n"; print PIE "%%Creationdate: ".$thisdate."\n"; if($legend eq "right" || $legend eq "left") {     print PIE "%%BoundingBox: $boundingxstart $ystart $boundingxend $yend\n\n"; }elsif($legend eq "bottom") { print PIE "%%BoundingBox: $boundingxstart $ystart $boundingxend $yend\n\n"; }else{ print PIE "%%BoundingBox: $xstart $ystart $xend $yend\n\n"; } print PIE "<< /PageSize [1000 1000] >> setpagedevice \n gsave\n"; print PIE "/typesize " . $valuetype . " def\n"; print PIE $ps; print PIE "%%set scale and translation\n"; print PIE "306 396 translate $hsize $vsize scale\n"; $thisstartangle=$startingangle; open(LABELANDCOLOR, "<$labelandcolor") or die("Couldn't open $labelandcolor\n"); $counter=0; while (<LABELANDCOLOR>) { chomp; $lclines[$counter]=$_; $counter++; } $counter=@lclines; my $labelx=0; my $labely=$counter*$vsize*9; $counter=0; foreach $lclines(@lclines){ ($label, $c, $m, $y, $k)=split(/\t/,$lclines); print PIE "%%draw pie chart\n";  $thisendangle=$thisstartangle+eval($thisslice);  if(eval($thisslice)>0){ $percent1num=int(100*eval($thisslice)/360);  if(((100*eval($thisslice)/36) % 10) >= 5){$percent1num++;}  $percent1=substr($percent1num,0)."%";  if($percent1num == 0){$percent1num=1;}  $wholepercent+=$percent1num;  if($wholepercent == 101){ $percent1num--;  $percent1=substr($percent1num,0) . "%";  }  print PIE "$c $m $y $k $thisstartangle $thisendangle ($percent1) pieslice\n";  if($legend eq "bottom"){  $leftright=0;  $labelx=-100;  $labely=-220-($slicecounter*$vsize*50);  print PIE "$c $m $y $k ($label) $labelx $labely $leftright  $hsize $vsize legend\n";  $thisstartangle=$thisendangle;  $thisslice=$slice[$slicecounter];  }elsif($legend eq "right"){  $labelx=(260/2) + 40;  $labely=$labely-28;  $counter++;  print PIE "$c $m $y $k ($label) $labelx $labely $hsize $vsize legend_right\n";  }elsif($legend eq "left"){ $labelx=-((260/2) + 40);  $labely=$labely-28; $counter++;  print PIE "$c $m $y $k ($label) $labelx $labely $hsize $vsize legend_left\n";  } }  $thisstartangle=$thisendangle;  $slicecounter++;  $thisslice=$slice[$slicecounter];  } my $exportstr="convert $filewithdir -density 1200x1200 $piedirectory$piename.$gexport"; print PIE "\n"; print PIE "showpage\n"; print PIE "grestore\n"; print PIE "%%trailer\n"; print PIE "%%EOF"; if(length($gexport)>0){`$exportstr`; }} close LABELANDCOLOR; close PIE;
}
sub column1{
...
=head1 SYNOPSIS
  ...
  #label and color files must be tab delimiter with CMYK color numbers and a string for the label
  #example: .23	.46	.73	0	Salary 
  ...
  #Valid formats: money ($) or d<number> for the number of decimal places. 
...
Copyright (C) 2013-2014 by Ken Owen
